...
class SDK
{
     ...
     * @return
     */
    public function AutoSignbyCA($signid, $email, $pagenum, $signx, $signy, $openflag = true, $sealname = '', $notice_urls = '')
    {
		...
		if (!empty($notice_urls))
		{
			$post_data['request']['content']['noticeUrls'] = $notice_urls;
		}
	 ...
	 * @return
	 */
	public function contractInfo($fsdid)
	{
		$method = __FUNCTION__ . ".json";
        $path = "/open/" . $method;
        
        //post data
        $post_data['request']['content']['fsdid'] = $fsdid;
        $post_data = json_encode($post_data);
	 ...
	 * @return
	 */
	public function contractQuerybyEmail($status, $email, $starttime, $endtime)
	{
		$method = __FUNCTION__ . ".json";
        $path = "/open/" . $method;
        
        //post data
        $post_data['request']['content']['status'] = $status;
		$post_data['request']['content']['email'] = $email;
		$post_data['request']['content']['starttime'] = $starttime;
		$post_data['request']['content']['endtime'] = $endtime;
        $post_data = json_encode($post_data);
	 ...
	 * @return
	 */
	public function endContract($signid)
	{
		$method = __FUNCTION__ . ".json";
        $path = "/open/" . $method;
     ...
     * @return
     */
    public function regUser($email, $mobile, $name, $user_type = Constants::USER_TYPE_PERSONAL, $throw_on_exists = '')
    {
        $method = __FUNCTION__ . ".json";
        $path = "/open/" . $method;
		...
		$post_data['request']['content']['throwOnExists'] = $throw_on_exists;
	...
	public function setTemplateVars($tid, $uid, array $vars)
	{
		$method = __FUNCTION__ . '.json';
		...
		$post_data['request']['content']['uid'] = "{$uid}";
		$post_data['request']['content']['vars'] = $post_vars;
        $post_data = json_encode($post_data);
	 ...
	 * @return
	 */
	public function uploadcontractly($tid, array $senduser, array $tempcontents, $font_size = 14, array $images = array())
	{
		$method = __FUNCTION__ . ".json";
        $path = "/open/" . $method;
		...
		if (!empty($images)) {
			$post_data['request']['content']['attachments']['images'] = array();
			foreach ($images as $image_data)
			{
				$post_data['request']['content']['attachments']['images'][] = base64_encode($image_data);
			}
		}
        $post_data = json_encode($post_data);
    ...
    public function uploadTemplate($uid, $name, $file_data, $description = '', $callbackurl = '', $typelist = '', $date = false, $text = false)
	{
		$method = __FUNCTION__ . '.json';
        $path = "/template/" . $method;
		...
		$post_data['request']['content']['uid'] = $uid;
		$post_data['request']['content']['name'] = $name;
		$post_data['request']['content']['fileData'] = base64_encode($file_data);
		$post_data['request']['content']['description'] = $description;
		$post_data['request']['content']['callbackurl'] = $callbackurl;
		$post_data['request']['content']['typelist'] = $typelist;
		$post_data['request']['content']['date'] = $date ? "1" : "0";
		$post_data['request']['content']['text'] = $text ? "1" : "0";
        $post_data = json_encode($post_data);
