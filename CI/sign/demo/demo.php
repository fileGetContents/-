...
function regUser()
{
    ...
    $user = randomReceiveUser();
    $email = $user['email'];
    $mobile = $user['mobile'];
    $name = $user['name'];
    $user_type = $user['usertype'];
    $response = $sdk->regUser($email, $mobile, $name, $user_type);
    addToLog('测试注册用户结果:');
    dump($response);
    var_dump($response);
}
...
function _regUser($email, $mobile, $name, $user_type = Constants::USER_TYPE_PERSONAL)
{
    global $sdk;
    $response = $sdk->regUser($email, $mobile, $name, $user_type, '1');
    return $response['response']['content']['uid'];
}
